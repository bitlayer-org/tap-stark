...
use crate::u32_std::{u32_compress, u32_equal, u32_equalverify, u32_push};
use crate::{BCAssignment, BabyBearU31, BfField};
    ...
    F: BfField + BitExtractor,
{
    ...
    fn new_from_fri_challenger(
        ...
        bc_assignment: &mut BCAssignment,
    ) -> FiatShamirSubTree<F> {
        assert_eq!(
    ...
    fn commit(&mut self, bc_assignment: &mut BCAssignment) {
...
fn new_u32_bit_commit(bc_assignment: &mut BCAssignment, value: U32) -> Commit {
    let bitcommit = bc_assignment.assign(U32_to_u32(value));
    let locking = script! {
        { bitcommit.checksig_verify_script() }
    };
    Commit {
        locking_script: locking,
        unlock_witness: bitcommit.signature(),
    }
}

fn new_challenge_commit<F>(bc_assignment: &mut BCAssignment, value: F) -> Commit
where
    F: BfField,
{
    let bitcommit: BitCommitment<F> = bc_assignment.assign_field(value);
    let locking = script! {
        { bitcommit.recover_message_at_stack() }
    };
    Commit {
        locking_script: locking,
        unlock_witness: bitcommit.signature(),
...
mod test {
    ...
    use p3_baby_bear::BabyBear;
    use p3_challenger::{
        BfChallenger, BfGrindingChallenger, Blake3Permutation, CanObserve, CanSample, CanSampleBits,
    };
    use p3_field::extension::BinomialExtensionField;
    use p3_field::{AbstractField, PermutationField, PrimeField32, U32};

    use super::{new_u32_bit_commit, Commit, FiatShamirSubTree, SubTree};
    use crate::bit_comm::pushable;
    use crate::fiat_shamir_subtree::new_challenge_commit;
    use crate::{execute_script, execute_script_with_inputs, to_digits, BCAssignment, BabyBear4};
