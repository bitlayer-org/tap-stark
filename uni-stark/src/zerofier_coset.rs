...
use p3_field::{
    batch_multiplicative_inverse, cyclic_subgroup_coset_known_order, PackedField, TwoAdicField,
};

/// Precomputations of the evaluation of `Z_H(X) = X^n - 1` on a coset `s K` with `H <= K`.
#[derive(Debug)]
pub struct ZerofierOnCoset<F: BfField> {
    /// `n = |H|`.
    log_n: usize,
    /// `rate = |K|/|H|`.
    rate_bits: usize,
...
impl<F: BfField> ZerofierOnCoset<F> {
