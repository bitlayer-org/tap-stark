...
use primitives::air::AirConstraintBuilder;
...
fn test_symbolic_prover(){
    ...
    let constraint_builder = gen_symbolic_builder(&FibonacciAir {}, 0, 3);
    let proof  = symbolic_prove(&config, constraint_builder, &mut challenger, trace, &pis);
    let constraint_builder1 = gen_symbolic_builder(&FibonacciAir {}, 0, 3);

    let permutation = Blake3Permutation {};
    let mut challenger = Challenger::new(permutation).unwrap();
    symbolic_verify(&config, constraint_builder1, &mut challenger, &proof, &pis).expect("verification failed");
